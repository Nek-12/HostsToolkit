...
// TODO: All the stuff reg. files should be put in one function
//  Don't show stats until file was saved/applied
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    connect(ui->CustomEntriesList, &QListWidget::itemActivated, ui->CustomEntriesList,
            &QListWidget::openPersistentEditor);
    connect(ui->CustomEntriesList, &QListWidget::currentItemChanged, ui->CustomEntriesList,
    ...
    connect(ui->AddUrlButton, &QPushButton::clicked, this, &MainWindow::add_url);
    load_custom();
}
...
auto MainWindow::prepare_file() -> std::string {
    ...
    if (ui->AddCreditsBox->isChecked())
        ret << CREDITS;
    ret << "\n# ------------- C U S T O M ------------- \n";
    for (const auto &l : customlines) ret << l->text().toStdString() << '\n'; // Add custom
    // process urls
    int i = 0;
    for (auto *url : urls) {
        ++i;
        msg(QString("Downloading %1").arg(url->text()));
        dl_mgr.do_download(url->text());
        //TODO: Broken, add threading (dang)
    }
    // process downloaded files
    qDebug() << "Downloads finished, starting processing\n";
    i = 0;
    ...
    if (ui->RemDupsBox->isChecked()) {
        ...
        while (ss) { // Process lines
            double j = double(strset.size()) / total_lines;
        ...
        ss.clear();
        total = strset.size();
        for (const auto &s : strset) ret << s << '\n'; // write every line back
    } else {                                           // don't remove duplicates
        std::vector<std::string> strvec;
        ret << "\n# ------------- MERGED TOGETHER ------------- \n";
        while (ss) { // Process lines
            double j = strvec.size() / double(total_lines);
            emit   progress(int(j / total_lines * 100));
            qDebug() << j;
            std::string line;
            std::getline(ss, line);
...
void MainWindow::update_stats() {
    ...
    ui->ApplyFileButton->setEnabled(pending);
    ui->SaveToButton->setEnabled(pending);
    qulonglong  lines = customlines.size(), comments = 0, seconds_to_load = 0;
    char        symbol = 0;
    std::string opt;
...
void MainWindow::del_selected_list_entry() { delete ui->CustomEntriesList->currentItem(); }
...
void MainWindow::msg(const QString &msg) { ui->Stats->showMessage(msg); }
